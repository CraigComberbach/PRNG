...
int True_16_bit_RNG_Generator(int channel, int *A2D_Buffer, int samples)
{
	...
	//It will work with fewer than 16 samples, though the maximum value will decrease accordingly
	//It will produce a signed 16-bit value
	int RNG = 0;

	for(;samples >= 0; --samples)
	{
		if(*(A2D_Buffer+samples) & 0x0001)
			RNG |= 1 << samples;
		else
			RNG &= ~(1 << samples);
	}
